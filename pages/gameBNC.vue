...
<template>
  <div class="game">
    ...
    <v-form ref="form" v-model="valid" class="game--form d-flex align-center">
      ...
      <v-btn :disabled="answerRight || showAnswer" @click="handleCheckAnswer">Test</v-btn>
    ...
    <div v-if="answerRight" class="game--result__right text-center amber--text text--lighten-3">
      Congratulations!! Your Answer is right!!
    </div>
    <div v-if="showAnswer" class="game--result__answer text-center">
      Answer is <span class="red--text text--lighten-3">{{ answer.join('') }}</span>
    </div>
  </div>
</template>
